...
#define MAX_ALLOWED		31	
#define MAX_EACHPLAYER 	31	
...
{	
	...
	hCvar_MachineRange 				= CreateConVar("l4d_machine_range", 				"9999", 	"Sets the max range of enemy detection by machine guns.", FCVAR_NOTIFY, true, 250.00, true, 3000.00);
	hCvar_MachineOverHeat 			= CreateConVar("l4d_machine_overheat", 				"300.0", 	"Sets the time before overheat in seconds.", FCVAR_NOTIFY, true, 5.00, true, 30.00);
	...
	hCvar_MachineAmmoCountGatling	= CreateConVar("l4d_machine_ammo_gatling_count",	"9999", 		"Sets the amount of ammo per gun", FCVAR_NOTIFY, true, 100.0, true, 10000.0);
	hCvar_MachineAmmoCount 			= CreateConVar("l4d_machine_ammo_count", 			"9999", 	"Sets the amount of ammo per gun", FCVAR_NOTIFY, true, 100.0, true, 10000.0);
	...
	hCvar_MachineHealth 			= CreateConVar("l4d_machine_health", 				"5000", 	"Sets the amount of health for each turret gun.", FCVAR_NOTIFY, true, 50.0, true, 1000.0);
	hCvar_MachineHealthGatling 		= CreateConVar("l4d_machine_health_gatling",		"5000", 		"Sets the amount of health for each gatling gun.", FCVAR_NOTIFY, true, 50.0, true, 1000.0);
	...
	hCvar_MachineAllowUse 			= CreateConVar("l4d_machine_allow_use", 			"0", 		"Allow using the turrets manually.\n0 = Disabled.\n1 = Enabled.", FCVAR_NOTIFY, true, 0.0, true, 1.0 );
	hCvar_MachineSleepTime 			= CreateConVar("l4d_machine_sleep_time", 			"9999.0", 	"Sets the max waiting time(seconds) to remain inactive when there are no enemies in range.", FCVAR_NOTIFY, true, 60.00, true, 600.00);
	hCvar_MachineFireRate 			= CreateConVar("l4d_machine_fire_rate",				"30", 		"Sest rate of fire, amount shots per second.", FCVAR_NOTIFY, true, 5.0, true, 30.0);
	hCvar_MachineFireRateGatling	= CreateConVar("l4d_machine_fire_rate_gatling", 	"30", 		"Sest rate of fire, amount shots per second.", FCVAR_NOTIFY, true, 5.0, true, 30.0);
	hCvar_MachineBlocking 	 		= CreateConVar("l4d_machine_blocking", 				"1", 		"If enabled, turrets are solid entities instead of walk-through. \n0 = Disabled.\n1 = Enabled.", FCVAR_NOTIFY, true, 0.0, true, 1.0 );
	hCvar_MachineGlowRange 			= CreateConVar("l4d_machine_glow_range", 			"25.0",		"Sets the amount of glow the turret produces", FCVAR_NOTIFY, true, 0.0, true, 100.0);
 	hCvar_MachineBetrayChance 		= CreateConVar("l4d_machine_betray_chance", 		"00.0",		"Sets the probability of being betrayed by machine guns", FCVAR_NOTIFY, true, 0.0, true, 100.0 );
	hCvar_MachineLimit 				= CreateConVar("l4d_machine_limit", 				"8", 		"Maximum turrets per user.", FCVAR_NOTIFY, true, 1.0, true, 16.0);
	hCvar_MachineLimitGatling		= CreateConVar("l4d_machine_gatling_limit", 		"8", 		"Maximum amount of gatling guns per user", FCVAR_NOTIFY, true, 1.0, true, float( MAX_EACHPLAYER ));	
...
//	if( IsTank( victim ) && !bLeft4DeadTwo )
	...
	if( iSpecialType == TYPE_TESLA && IsInfected( victim ) )
		CreateTimer( 0.033, DissolveCommonDelay, EntIndexToEntRef( victim ), TIMER_FLAG_NO_MAPCHANGE );
}
...
void CreatePushTimer( int client, int entity )
{
	if( GetEntPropEnt( client, Prop_Send, "m_hUseEntity" ) )
	{
		if( MachineGunerTime[client] + 1.0 < GetEngineTime() )
		{
			...
			CreateTimer( 0.033, UserPushTimer, hPack, TIMER_FLAG_NO_MAPCHANGE ); 
		}
	}
}
...
//	ShoveClient( client );
	...
	CreateTimer( 0.033, ShowEnergyEffects, hPackEffects, TIMER_FLAG_NO_MAPCHANGE );
	return Plugin_Continue;	
}
...
/*
//test 360 degree turret targeting
int IsEnemyVisible(int iEntity, int iNewTarget, float vStartingPos[3], float vEndPos[3], float vAng[3], int iTeam) {
    ...
    // Perform multiple ray traces in different directions
    float angles[8][3] = {
        {0.0, 0.0, 0.0},
        {0.0, 90.0, 0.0},
        {0.0, 180.0, 0.0},
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (TR_DidHit(hTrace)) {
            ...
            if (target == iNewTarget) {
                return 1; // Target is visible in one of the directions
            }
        }
    }

    return 0; // Target is not visible in any direction
}
*/
//original DLR ray traced turret eye fov targeting system
int IsEnemyVisible( int iEntity, int iNewTarget, float vStartingPos[3], float vEndPos[3], float vAng[3], int iTeam )
{
	if( iNewTarget <= 0 ) 
		return 0;
...
stock bool IsVisibleTo(float position[3], float targetposition[3])
{
	...
	if( TR_DidHit(trace) )
	{
		...
		if( GetVectorDistance(position, vStart) + 1.0 >= GetVectorDistance(position, targetposition) )
